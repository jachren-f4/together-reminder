/**
 * Reset Database with Test Couple - Linked Puzzle 2 Ready
 *
 ...
 * - Linked: puzzle_001 completed, puzzle_002 ready to play
 * - Word Search: locked (unlocks after Linked)
 *
 * This allows testing:
 * 1. Linked puzzle_002 (for hint/dual-direction clue testing)
 ...
 *
 * Usage:
 *   npx tsx scripts/reset_test_couple_linked_puzzle2.ts
 */
...
async function createTestData(): Promise<void> {
  ...
  try {
    ...
    // 8. Create COMPLETED Linked match for puzzle_001 so puzzle_002 is next
    await client.query(
      `INSERT INTO linked_matches (
        id, couple_id, puzzle_id, branch, status, board_state, current_rack,
        ...
        player1_id, player2_id, winner_id, created_at, completed_at
      ) VALUES (
        gen_random_uuid(), $1, 'puzzle_001', 'casual', 'completed',
        '{}', '{}', $2, 20, 150, 140, 0, 0, 30, 30,
        $2, $3, $2, NOW() - interval '1 day', NOW() - interval '1 day'
      )`,
      [TEST_COUPLE_ID, pertsaId, kiluId]
    );
    console.log('   ✓ linked_matches: puzzle_001 (completed, Pertsa won)');

    // 9. Create LP transaction records
    const lpSources = [
...
async function main() {
  console.log('\n' + '='.repeat(60));
  console.log('   RESET WITH TEST COUPLE - LINKED PUZZLE 2 READY');
  ...
  console.log('      - Linked puzzle_001: ✅ completed (Pertsa won)');
  console.log('      - Linked puzzle_002: 🆕 ready to play (casual branch)');
  console.log('      - Word Search: 🔒 locked (unlocks after Linked)');
  console.log('      - LP: 150');

  console.log('\n   Testing flow:');
  console.log('      1. Login as Pertsa or Kilu → daily quests show COMPLETED');
  console.log('      2. Linked game card should show puzzle_002');
  console.log('      3. Test hint functionality and dual-direction clues');
