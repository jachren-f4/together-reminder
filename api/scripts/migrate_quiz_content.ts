 ...
 *
 * Run with: npx ts-node scripts/migrate_quiz_content.ts
 */
...
function generateClassicQuizzes() {
  ...
  const allQuestions: ClassicQuestion[] = JSON.parse(rawData);

  // Group by category
  const byCategory: Record<string, ClassicQuestion[]> = {};
  ...
  for (const [branch, categories] of Object.entries(CLASSIC_BRANCHES)) {
    ...
    const branchQuestions: ClassicQuestion[] = [];
...
function generateAffirmationQuizzes() {
  ...
  const allQuizzes: AffirmationQuiz[] = data.quizzes;
...
function generateYouOrMeQuizzes() {
  ...
  const data = JSON.parse(rawData);
  const allQuestions: YouOrMeQuestion[] = data.questions;

  // Group by category
  const byCategory: Record<string, YouOrMeQuestion[]> = {};
  for (const q of allQuestions) {
    if (!byCategory[q.category]) {
      byCategory[q.category] = [];
    }
  ...
  for (const [branch, categories] of Object.entries(YOUORME_BRANCH_MAPPING)) {
    ...
    // Collect questions for this branch
    const branchQuestions: YouOrMeQuestion[] = [];
    for (const category of categories) {
      if (byCategory[category]) {
        branchQuestions.push(...byCategory[category]);
      }
