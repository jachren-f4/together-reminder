/**
 * Wipe test accounts by email
 *
 * This completely removes users from the database, including:
...
async function findUsersByEmail(emails: string[]): Promise<{ id: string; email: string }[]> {
  console.log('\n🔍 Looking up users by email...\n');

  const users: { id: string; email: string }[] = [];

  try {
    // Query auth.users directly via the database pool (more reliable than admin API)
...
async function main() {
  console.log('\n' + '='.repeat(60));
  console.log('   WIPE TEST ACCOUNTS');
  ...
  // Find users
  const users = await findUsersByEmail(TEST_EMAILS);

  if (users.length === 0) {
    console.log('\n⚠️  No matching users found in auth.users');
    console.log('   (They may have already been deleted or never existed)\n');
    process.exit(0);
  }
  ...
  try {
    ...
    console.log('\n' + '='.repeat(60));
    console.log('✅ Test accounts wiped successfully!');
    console.log('='.repeat(60));
    console.log('\n📝 Note: Also uninstall the apps on both phones to clear local data\n');
